...
export default function SortCode({ value, valueLength, onChange }: Props) {
  ...
  const inputOnChange = (
    ...
    const newValue =
      value.substring(0, idx * 2) + targetValue + value.substring(idx * 2 + 2)
    onChange(newValue)
  ...
  const inputOnKeyDown = (
    ...
    if (e.key === "Backspace" && target.value.length === 1) {
      const newValue =
        value.substring(0, idx * 2) + value.substring(idx * 2 + 1)
      target.value = ""
      onChange(newValue)
      return
    }
